...
// Get and sanitize the target task ID
$task_id = (int) $_POST['task_id'];

// Handle form submission to update an existing task
if (isset($_POST['button'])) {

    // Fetch the current task data before making changes (for change log)
    ...
    // Sanitize all incoming task fields
    ...
    // Apply the updates to the task record
    ...
    // Build a list of changed fields for the activity log
    $changes = [];
    if ($before) {
        $fields = [
            'task_title'  => 'title',
    ...
    // Log the update activity with a summary of what changed
